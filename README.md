...
New plans use the official starting budget. Imported plans use current market prices plus published bank as a **planning estimate**. Enable “Transfer budget & points hits” on a complete squad to save a starting squad, enter bank and remaining free transfers, and override purchase/selling prices. Selling overrides take precedence; purchase prices apply half-profit rounding and full losses. Missing prices are labelled market estimates. Final squad changes determine planned transfers and four-point hits beyond the entered allowance; unknown free transfers leave net xPts unknown. Affordability and budget checks use these values. Previously confirmed transfers/hits, Wildcard/Free Hit, automatic substitutions and historical prices are not simulated. Bench arrows reorder outfield substitutes while keeping the goalkeeper first. Undo/redo retains 50 edits per plan for the current session; draft data and financial settings persist locally. Shared links exclude financial history/settings and carry the resulting planning budget.
